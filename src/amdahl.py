    ...
    if serial:
        if serial < 0.0 or serial > 1.0:
            raise ArithmeticError(
                "Invalid argument for serial, must be decimal percentage"
            )
        parallel = 1.0 - serial
    else:
        if parallel < 0.0 or parallel > 1.0:
            raise ArithmeticError(
                "Invalid argument for parallel, must be decimal percentage"
            )
        serial = 1.0 - parallel

    return 1.0 / (serial + (parallel) / cores)
...
def parse_args() -> Namespace:
    ...
    parser.add_argument(
        "--n", default=None, type=int, help="The number of cores available on the CPU"
    )
...
def main() -> None:
    ...
    print(
        ...
        percent parallel on a CPU with {} cores <= {:.2f}""".format(
            args.s * 100.0 if args.s else 100.0 * (1.0 - args.p),
            args.p * 100.0 if args.p else 100.0 * (1.0 - args.s),
            args.n,
            speedup,
        )
    )
